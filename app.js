...
const app = express();
const port = 3000;
const productData = [{"Productname":"Mobile"},
    {"Productname": "Car"},
    {"Productname": "Bike"},
    {"Productname": "Bus"}
];
app.get('/home', function(req ,res){
    const number1 = 33;
    console.log(`Hello World ${number1}`);
    res.send (`Hello 3 ${number1}`);
});
//home screen
...
app.post(`/addusers`,  (req,res) => {
    ...
    users.push(req.body)
    res.status(200).send({status:true,  message: "Users Data Added",data:users}); //this in not run in localost
});

app.put(`/add-users`,  (req,res) => {
    const querydata = req.query;
    res.status(401).send({status:true,  message: "Users Data Added",data:req.query}); //this in not run in localost
});
...
app.patch(`/update-users`,  (req,res) => {
    const querydata = req.query;
    res.status(20).send({status:true,  message: "Users Data Added",data:req.query}); //this in not run in localost
});
...
app.post(`/post/1/comments`, (req,res) => {
  res.send(posts)  //this in not run in localost
});
// comments screen
...
app.listen(port ,()=> {
    console.log(`Server is running http://localost:${port} port`)
});
